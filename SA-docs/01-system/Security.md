...
| Browser custody | `httpOnly` + `Secure` + `SameSite=Lax` cookie (`Strict` for admin routes), scoped path, explicit expiry. **No token is readable by client JavaScript; nothing in `localStorage`** | [ADR-0025](./ADR/ADR-0025-httponly-cookie-session.md) §4 |
...
| Non-browser callers | `Authorization: Bearer <jwt>`. A new caller, not new rules | [IC §5](../04-shared/Integration%20Contract.md) |
...
| Signing algorithm | Asymmetric, so a verifier never holds the signing key. `CON-08` anticipates extraction into services; symmetric signing would make every extracted service a custodian of the secret | **[ASSUMPTION]** `EdDSA` (Ed25519), or `RS256` where library support requires it |
