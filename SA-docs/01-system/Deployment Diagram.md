...
| **Metrics** | Actuator/Micrometer on the management port only: `http.server.requests` for request throughput, latency, and errors, plus `ecp.catalog.cache.accesses` for catalog cache-aside reads. The latter has the fixed tag `cache=catalog` and `result=hit`, `miss`, or `error`; it is incremented once for each cache access. No Actuator endpoint is exposed on the public API port. | `NFR-OBS-04` |
