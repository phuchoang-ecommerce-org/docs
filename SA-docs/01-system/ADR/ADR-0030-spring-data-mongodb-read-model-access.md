...
- **The applied-event ring is bounded, so dedupe is bounded.** A redelivery arriving after the ring has rotated past its `eventId` will double-count. The window must exceed Kafka's maximum redelivery horizon — which `Database.md` §11 already flags as deferred to `Backend Architecture.md`, so this record inherits an open dependency rather than resolving one.
