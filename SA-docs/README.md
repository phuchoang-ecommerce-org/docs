...
Folders are organized by concern, following the target layout in [`example-folder-structure.md`](./example-folder-structure.md). Folders from that layout with no content yet (`00-vision/`, `Sequence/`) are omitted until there is something to put in them, rather than checked in empty. `04-shared/OpenAPI/` was reserved-and-empty on that basis until [`ADR-0031`](./01-system/ADR/ADR-0031-contract-first-openapi.md) filled it.
