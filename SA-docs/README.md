...
| [`01-system/ADR/`](./01-system/ADR) | `ADR-0001`–`ADR-0003`, `ADR-0005`–`ADR-0039` | One record per decision: context, alternatives considered, outcome, consequences — 5 cross-cutting, 20 backend, 13 frontend. `ADR-0004` is retired, not reused ([why](./01-system/ADR/README.md)) |
...
| [`03-frontend/`](./03-frontend) | [`Frontend Architecture.md`](./03-frontend/Frontend%20Architecture.md) | The frontend hub — the `ecp-web` runtime and its position in the trusted computing base, the four-layer model and the Server/Client boundary, session custody (the `ecp_session` cookie, signed double-submit CSRF, serialised refresh), the Content Security Policy, and the TypeScript/codegen/lint/format toolchain. Indexes the five documents below |
| [`03-frontend/`](./03-frontend) | [`Routing.md`](./03-frontend/Routing.md) | The route map — four route groups and their layout composition, the four rendering classes, every storefront, auth, account, and admin route with the OpenAPI operations behind it, boundary placement, the closed list of route handlers, and a coverage assertion against all 155 operations |
| [`03-frontend/`](./03-frontend) | [`Feature Structure.md`](./03-frontend/Feature%20Structure.md) | The folder layout — one feature per API domain, the four-layer dependency order, the eight import rules a lint gate enforces, and where the fourteen frontend domains do not line up with the thirteen backend modules |
| [`03-frontend/`](./03-frontend) | [`Data Fetching.md`](./03-frontend/Data%20Fetching.md) | The fetch client and what it attaches, boundary parsing, the zero-lag rule, the closed list of four client-cached cases, Server Actions and idempotency, event-driven revalidation, cursor pagination, and the error-code-to-screen mapping |
| [`03-frontend/`](./03-frontend) | [`State Management.md`](./03-frontend/State%20Management.md) | Where each category of state lives, the URL search-param encoding as a stability contract, the four-slice client store, and what never lives in client state |
| [`03-frontend/`](./03-frontend) | [`Performance.md`](./03-frontend/Performance.md) | Per-route-class client budgets and why they are not `NFR-PERF-01`, bundle/image/font discipline, streaming and boundary placement, the CSP-nonce-versus-static-generation trade, and how the budgets are measured and gated |
...
03-frontend/Frontend Architecture.md the runtime those decisions run in -- layers, session, CSRF, CSP, toolchain
03-frontend/Feature Structure.md     where code goes, and what it may import
03-frontend/Data Fetching.md         how a screen gets its data -- and how a failure becomes a screen
03-frontend/Routing.md               every route, its rendering class, and the operations behind it
03-frontend/State Management.md      where state lives -- URL, server, local, store
03-frontend/Performance.md           what each route class costs a customer, and what enforces it
03-frontend/UI Design System.md      what it looks like -- space, type, colour, motion, accessibility
      ↓
