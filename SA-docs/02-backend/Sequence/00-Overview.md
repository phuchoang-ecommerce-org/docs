...
**What this costs.** Removing the compile-time link between publisher and consumer is what keeps the module graph acyclic, and the price is paid in full here: **nothing in the compiler catches a consumer that misreads a field.** Contract tests against the published schema are the only mechanism that would, and [ADR-0032](../../01-system/ADR/ADR-0032-json-event-serialisation-and-schema-contract.md) §4 now names them: JSON Schema files in `04-shared/Event Contract/`, checked by a publisher-side test against a real outbox row **and** a consumer-side test asserting that every field a consumer binds is one its publisher promised. The second is the one that spans this gap. Enforcement is CI-time rather than publish-time, which is the residue [ADR-0032](../../01-system/ADR/ADR-0032-json-event-serialisation-and-schema-contract.md) §5 accepts in exchange for not running a schema registry.
